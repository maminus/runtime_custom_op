...
std::vector<std::tuple<std::string, OrtCustomOp*>> op_list = {
	std::make_tuple(std::string(domain_base) + "." + "float", &op_fma_float),
	std::make_tuple(std::string(domain_base) + "." + "double", &op_fma_double),
};
...
OrtStatus* ORT_API_CALL RegisterCustomOps(OrtSessionOptions* options, const OrtApiBase* api_base) {
	...
	for (auto& name_and_op: op_list) {
		std::string domain_name;
		OrtCustomOp* op;
		std::tie(domain_name, op) = name_and_op;
